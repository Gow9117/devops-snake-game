...
class GameAppIntegrationTest {
    ...
    @Autowired private PasswordEncoder passwordEncoder;   // inject interface, not concrete class
    ...
    @Test
    @DisplayName("POST /register with blank username returns validation error")
    void integrationTest_Register_BlankUsername_ReturnsError() throws Exception {
        mockMvc.perform(post("/register").with(csrf())
                .param("username",        "  ")
