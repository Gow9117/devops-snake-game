...
class AuthControllerTest {
    ...
    @Test
    @DisplayName("POST /register with username longer than 50 chars returns error")
    void postRegister_TooLongUsername_ReturnsError() throws Exception {
        String longUsername = "a".repeat(51);
        mockMvc.perform(post("/register").with(csrf())
                .param("username",        longUsername)
                .param("email",           "toolong@e.com")
                .param("password",        "password123")
                .param("confirmPassword", "password123"))
            .andExpect(status().isOk())
            .andExpect(view().name("register"))
    ...
    // ===== POST /register — blank username =====

    @Test
    @DisplayName("POST /register with blank username returns error")
    void postRegister_BlankUsername_ReturnsError() throws Exception {
        mockMvc.perform(post("/register").with(csrf())
                .param("username",        "   ")
                .param("email",           "user@e.com")
                .param("password",        "password123")
                .param("confirmPassword", "password123"))
            .andExpect(status().isOk())
            .andExpect(view().name("register"))
