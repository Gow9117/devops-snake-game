...
class DashboardControllerTest {
    ...
    // Prevent Spring from instantiating BCryptPasswordEncoder directly
    @MockitoBean
    ...
    @Test
    @WithMockUser(username = "firstonly")
    @DisplayName("GET /dashboard sets displayName from firstName only when lastName is null")
    void getDashboard_SetsDisplayName_FirstNameOnly() throws Exception {
        User firstOnly = new User("firstonly", "fo@e.com", "enc");
        firstOnly.setFirstName("Solo");
        // lastName intentionally left null

        when(userService.findByUsername("firstonly")).thenReturn(Optional.of(firstOnly));
        when(userService.getTotalUserCount()).thenReturn(1L);

        mockMvc.perform(get("/dashboard"))
            .andExpect(status().isOk())
            .andExpect(model().attribute("displayName", "Solo"));
    }

    @Test
    @WithMockUser(username = "noname")
    @DisplayName("GET /dashboard handles user with no first/last name (falls back to username)")
